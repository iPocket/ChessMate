...
import me.protoflicker.chessmate.protocol.enums.GameResult;
...
public final class ParticipationTable {
	...
	public static GameResult getResult(byte[] participationId){
		String statement =
				"""
				SELECT result
				FROM `Participations`
		...
		try (PreparedStatement s = Server.getThreadDatabase().getConnection().prepareStatement(statement)){
			...
			ResultSet r = s.executeQuery();
			if(r.next()){
				return GameResult.getByCode(r.getInt(1));
			} else {
				return null;
			}
		} catch (SQLException e){
